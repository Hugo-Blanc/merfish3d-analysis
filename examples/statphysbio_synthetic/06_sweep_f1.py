...
def sweep_decode_params(
    root_path: Path,
    gt_path: Path,
    spotmap_threshold_range: tuple[float] = (0.1, 0.6),
    spotmap_threshold_step: float = 0.05,
    ...
    for spotmap in spotmap_values:
        for mag in mag_values:
            ...
            try:
                print(time_stamp(), f"spotmap threshold: {round(spotmap,2)}; magnitude threshold: {round(mag,2)}")
                decode_pixels(
                    root_path=root_path,
